...
export default function CategoryModal({ isDarkMode, isOpen, categoryToEdit, onClose, onSave }) {
  const [form, setForm] = useState({ name: '', icon_key: 'Package', color: 'blue' });

  useEffect(() => {
    if (categoryToEdit) {
      setForm({ 
        name: categoryToEdit.name, 
        icon_key: categoryToEdit.icon_key || 'Package',
        color: categoryToEdit.color || 'blue'
      });
    } else {
      setForm({ name: '', icon_key: 'Package', color: 'blue' });
    }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 sm:p-6">
      ...
      <div className={`relative w-full h-full sm:h-auto sm:max-w-2xl border-0 sm:border rounded-none sm:rounded-[4rem] shadow-3xl overflow-y-auto ring-1 ring-white/10 ${isDarkMode ? 'bg-slate-900 sm:border-slate-800 shadow-black/50' : 'bg-white sm:border-slate-200 shadow-slate-300'}`}>
        <div className="p-6 sm:p-12 lg:p-16">
          <h2 className={`text-2xl sm:text-3xl lg:text-4xl font-black tracking-tighter uppercase italic mb-3 sm:mb-4 leading-tight ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{categoryToEdit ? 'Editar Categoria' : 'Nova Categoria'}</h2>
          <p className="text-[10px] font-black text-blue-500 uppercase tracking-[0.4em] mb-8 sm:mb-12">Setor Operacional</p>
          <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-10">
            <div className="space-y-4">
              <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest px-1">Nome</label>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {WORKSPACE_ICONS.slice(0, 8).map(icon => (
