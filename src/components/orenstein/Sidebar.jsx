      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-10 custom-scrollbar">
        ...
        <section>
          ...
          <div className="space-y-4">
            <div className={`w-full flex items-center gap-4 px-5 py-5 rounded-[2.5rem] border ${isDarkMode ? 'bg-blue-600 border-blue-400 shadow-xl text-white' : 'bg-blue-50 border-blue-200 text-blue-800 shadow-sm'}`}>
              <div className={`p-2 rounded-xl ${isDarkMode ? 'bg-white/20' : 'bg-white shadow-sm'}`}>
                {ICON_MAP[currentWorkspace?.icon_key] || <Box className="w-5 h-5" />}
              </div>
              <span className="text-xs font-black uppercase tracking-wide truncate flex-1">{currentWorkspace?.name || 'Sem workspace'}</span>
            </div>
